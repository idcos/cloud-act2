            ...
            while proc.has_unread_data:
                try:
                    try:
                        ...
                        # eof of stdout
                        if proc.flag_eof_stdout:
                            break

                        if kwargs.get('__report__'):
                            jid = kwargs.get('__jid__')
                            target_id = kwargs.get('__id__')
                            act2_proxy_server = kwargs.get('act2_proxy_server')
                            # data change and callback exists
                            if (cstdout or cstderr) and act2_proxy_server:
                                report_data = {
                                    'stdout': stdout,
                                    'stderr': stderr,
                                    'jid': jid,
                                ...
                                try:
                                    resp = requests.post(act2_proxy_server, json=report_data)
                                    log.debug('request to %s response %s', act2_proxy_server, resp)
                                except Exception as e:
                                    log.error("post to proxy error %s", e)
