    ...
    def _thread_return(cls, minion_instance, opts, data):
        ...
        if function_name in minion_instance.functions:
            try:
                ...
                if data['fun'] == 'cmd.script':
                    log.debug('opts %s, data %s', opts, data)
                    kwargs['__jid__'] = data['jid']
                    target_id = opts.get('idcos_system_id', opts.get('uuid', opts.get('id')))
                    kwargs['__id__'] = target_id
                    kwargs['__report__'] = True
