    ...
    if mswindows:
        ...
        def send_signal(self, sig):
            ...
            if sig == signal.SIGTERM:
                ...
                raise ValueError('unsupporteded signal: {0}'.format(sig))
            # pylint: enable=E1101
    ...
    # <---- Windows Methods --------------------------------------------------
        ...
        def _spawn(self):
            ...
            if self.pid == 0:
                ...
                try:
                    os.closerange(pty.STDERR_FILENO + 1, exec_err_pipe_write)
                    os.closerange(exec_err_pipe_write+1, max_fd[0])
                except OSError:
                    pass
                ...
                os.close(exec_err_pipe_read)
                if exec_err_data:
                    raise Exception('subprocess exec error %s', exec_err_data)
